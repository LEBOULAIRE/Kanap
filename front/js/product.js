...
function addToCart (){    
        ...
        if ( quantityPageValue < 1 || colorPageValue === '') {
            ...
            if (produitLocalStorageProduct){
                const result = produitLocalStorageProduct.find (  (search) => search.id === idProduct && search.color === colorPageValue);
                if (result){
                    let moreQuantity = parseInt(objetProduct.quantity) +  parseInt(result.quantity);
                    if (moreQuantity <101) {
                    result.quantity = moreQuantity;
                    alert (`Nous avons ${moreQuantity} produits dans notre panier `)
                    localStorage.setItem("produit", JSON.stringify(produitLocalStorageProduct)) }
                    else {alert ('La quantité est malheuresment trop grande pour votre commande ') }
               }
               else {
